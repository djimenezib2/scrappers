...
# Licitacion Contratacion Del Estado
class LCDE:
    ...
    def __init__ (self, url, soup, match):
        ...
        self.numero_licitadores                                 = self.repair_BiddersNumber(self.find_text_by_id(soup, "span", "text_NumeroLicitadores"))
    ...
    def repair_BiddersNumber(self, biddersNumber):
        if not biddersNumber.isdigit() and not biddersNumber == '':
            return None
        else:
    ...
    def get_pdf_link(self):
        for el in self.documents:
            if(el['name'] == 'Anuncio de Licitación'):
                for doc in el['documents']:
                    if(doc['name'] == 'Pdf'):
                        return doc['url']
